...
RESOURCE_FOLDER = os.path.join(os.path.dirname(os.path.dirname(__file__)), "res")
def build_table_html(
    ...
    env = jinja2.Environment(
        loader=jinja2.FileSystemLoader(os.path.join(RESOURCE_FOLDER, "templates")),   
        #autoescape=jinja2.select_autoescape(),                                              
        trim_blocks=True,                                                                    
        lstrip_blocks=True,                                                                  
    )
    template = env.get_template("template-assets.html")
    html_page = template.render(data=data, data_info=data_info, data_columns=data_columns, resource_folder=RESOURCE_FOLDER)
